...
vmrProject = configs.bvqx.CreateDocumentVMR(...
    filetype,...
    firstFileName,...
    nrSlices,...
    swap,...
...
[~,proj_]=fileparts(projectPath);

vmrProject.SaveAs( fullfile ...
    (projectPath,...
    'PROJECT',...
    [configs.filesSignature '_' proj_ '.vmr']));


% Create .pos file.

vmrFile_temp=xff(fullfile ...
    (projectPath,...
    'PROJECT',...
    [configs.filesSignature '_' proj_ '.vmr']));

[success] = createPosFile (vmrFile_temp,...
    fullfile ...
    (projectPath,...
    'PROJECT',...
    [configs.filesSignature '_' proj_]));

vmrFile_temp.ClearObject;
clear vmrFile_temp;
