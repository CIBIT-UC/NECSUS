...
%% open FmrProject
runName=fmrProjectNamePath;
% runName =  strrep(runName{1},'_','-');

alignRun=configs.firstFunctRunName;

fmrProjectName = fullfile( configs.dataRootSession,...
    'func',...
    fmrProjectNamePath,...
    'PROJECT',...
    'PROCESSING',...
    [ configs.filesSignature '_' runName '_bold.fmr']);
...
fmrProject.CorrectMotionTargetVolumeInOtherRunEx(...
    fullfile( configs.dataRootSession,...
        'func',...
        (configs.firstFunctRunName),...
        'PROJECT',...
        'PROCESSING',...
        [ configs.filesSignature  '_' alignRun '_bold.fmr']),...
