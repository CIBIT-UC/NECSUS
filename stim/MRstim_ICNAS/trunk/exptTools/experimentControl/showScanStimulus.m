...
for frame = 1:nFrames	
    ...
    if stimulus.seq(frame)>0
        ...
        elseif strcmp(display.arch, 'maci') && isfield(stimulus, 'cmap_original')
            cmap=stimulus.cmap;
        ...
        elseif isfield(stimulus, 'cmap_original')
            Screen('DrawTexture', display.windowPtr, stimulus.textures(imgNum), stimulus.srcRect, stimulus.destRect);
            if isfield(stimulus, 'seq2') && stimulus.seq2(frame)>0
                Screen('DrawTexture', display.windowPtr, stimulus.textures(stimulus.seq2(frame)), stimulus.srcRect, stimulus.destRect);
            end
        else
            Screen('DrawTexture', display.windowPtr, stimulus.textures(imgNum), stimulus.srcRect, stimulus.destRect);
        end
    ...
    while(waitTime<0),
        % Scan the SV6011 device for subject response
        [ssKeyCode,ssSecs] = deviceSV6011('response',display.devices.SV6011port);
        if(ssKeyCode(1)~=0)
%            kc = find(ssKeyCode);
