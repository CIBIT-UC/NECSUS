...
for stimNum = 1:length(stimulus)
	...
	% make textures
    if ~strcmp(display.arch, 'maci') && isfield(stimulus, 'cmap_original')
        nImages=size(stimulus(stimNum).images,3)./4;
        for imgNum = 1:nImages,
            stimulus(stimNum).textures(imgNum) = Screen('MakeTexture',display.windowPtr, double(stimulus(stimNum).images(:,:,(((imgNum-1)*4)+1):((imgNum*4)))));
        end
        removeImages=1;
    else
        for imgNum = 1:nImages,
...
Screen('DrawTexture', display.windowPtr, stimulus(1).textures(imgNum), ...
	stimulus(1).srcRect, stimulus(1).destRect);
return
