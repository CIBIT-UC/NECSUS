...
%%
figure(2)
plot(1:length(answers),answers(:,2),'-');
hold on,
[trueidxs]=find(answers(:,3)==1);
[falseidxs]=find(answers(:,3)==0);
plot(trueidxs,answers(trueidxs,2),'bo');
plot(falseidxs,answers(falseidxs,2),'b.', 'MarkerSize', 20);
xlabel('iteration');
ylabel('contrast [%]');
