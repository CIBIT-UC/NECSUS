function [  ] = runStim( ptb, lcd, gabor, methodStruct )
...
try
    ...
    linearCLUT=Screen('LoadNormalizedGammaTable',window,repmat(invertedCLUT, [3,1])' ); % upload inverse gamma function to screen - linearize lum.
    ...
    gabor.gabortex = CreateProceduralGabor(window,...
        ...
        stim.gaborDimPix,...
        0,...
        [normlzInput,normlzInput,normlzInput,0.0]);
    ...
    % If first contrast present countdown.
    ...
    for trial = 1:nTrials
        ...
        if ~sCrit
            ...
            for frame=1:presTimeFrames
                ...
                Screen('DrawTexture', window, gabortex, [], [], angle, [], [], ...
                    [], [], kPsychDontDoRotation, [phase+180, spatFreq, sigma, methodStruct.contrastTrial, aspectratio, 0, 0, 0]);
            ...
            while respToBeMade
                ...
                elseif keyCode(keyView)
                    respMatrix(trial,:) = [trial,methodStruct.contrastTrial ,1];

                    respToBeMade = false;
                    % Update the pdf
                    response=1;
                    methodStruct.q=QuestUpdate(methodStruct.q,methodStruct.contrastTrial,response); % Add the new data (actual test intensity and observer response) to the database.
                    
                elseif keyCode(keyNotView)
                    respMatrix(trial,:) = [trial,methodStruct.contrastTrial ,0];
        ...
        last=methodStruct.contrastTrial;
